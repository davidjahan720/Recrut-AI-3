...
test.describe('Page Candidatures', () => {
  ...
  test('les filtres de statut sont présents', async ({ page }) => {
    ...
    await expect(page.locator('button', { hasText: /En attente/ })).toBeVisible()
  })

  test('ouvre le dialog de dépôt de CV', async ({ page }) => {
    await page.click('button:has-text("Déposer des CV")')
    await expect(page.locator('[role="dialog"]')).toBeVisible()
    await expect(page.locator('text=Sélectionner une offre active')).toBeVisible()
  })
  ...
  test('boutons Voir / PDF / Envoyer analyse présents', async ({ page }) => {
    const rows = page.locator('tbody tr')
    if (await rows.count() === 0) test.skip()
    const first = rows.first()
    if (await first.locator('text=Aucune candidature').isVisible()) test.skip()
    await expect(first.locator('button', { hasText: 'Voir' })).toBeVisible()
    await expect(first.locator('button', { hasText: 'PDF' })).toBeVisible()
    await expect(first.locator('button', { hasText: 'Envoyer analyse' })).toBeVisible()
  })
})
...
test.describe('Dashboard', () => {
  ...
  test('affiche les titres graphiques', async ({ page }) => {
    ...
    await expect(page.locator('text=Répartition des candidatures')).toBeVisible()
  })
})
